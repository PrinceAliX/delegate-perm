...
# Help function
usage() {
  echo "Usage: $0 -token <access_token_file> -delegate <delegate_sa> -file <service_account_list>"
  exit 1
}
...
if [[ -z "$TOKEN_FILE" || -z "$DELEGATE" || -z "$SERVICE_ACCOUNTS_FILE" ]]; then
...
while read -r SA; do
  ...
  REQ=$(jq -cn --arg del "$DELEGATE" --arg payload "$CLAIMS" '{delegates:[$del],payload:$payload}')
  ...
  #### ----------- signBlob -----------
  echo -n "[*] signBlob... "
  REQ=$(jq -cn --arg del "$DELEGATE" --arg payload "$MESSAGE" '{delegates:[$del],payload:$payload}')
  RESP=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
               -H "Content-Type: application/json" \
               -X POST --data "$REQ" \
  ...
  REQ=$(jq -cn --arg del "$DELEGATE" --argjson scopes "[\"$SCOPES\"]" '{delegates:[$del],scope:$scopes}')
  RESP=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
               -H "Content-Type: application/json" \
               -X POST --data "$REQ" \
  ...
  REQ=$(jq -cn --arg del "$DELEGATE" --arg aud "$AUDIENCE" '{delegates:[$del],audience:$aud,includeEmail:true}')
  RESP=$(curl -s -H "Authorization: Bearer $ACCESS_TOKEN" \
               -H "Content-Type: application/json" \
               -X POST --data "$REQ" \
